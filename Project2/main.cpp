...
bool Setup()
{
	...
	// 设置摄像机
	D3DXVECTOR3 position(0.0f, 0.0f, -150.0f);
...
bool Display(float timeDelta)
{
	if (Device)
	{
		...
		float ori = 5.0f;
		float static rt = ori;
		D3DXMatrixTranslation(&Rt, rt, 0, 0);
		float maxT = 150.0f;
		float speed = 50.0f;

		bool static flg = true;
		if (rt > (ori + maxT) || rt < (ori - maxT))
		{
			rt = flg ? (ori + maxT) : (ori - maxT);
			flg = !flg;
		}
		else {
			if (flg)
		...
		Device->SetTransform(D3DTS_WORLD, &Rt);
